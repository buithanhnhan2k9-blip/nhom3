...
// ==========================================================================
// 1. CẤU HÌNH KẾT NỐI DATABASE (Thay đổi thông số nếu cần)
// ==========================================================================
$host = 'localhost';
...
// ==========================================================================
// 2. ĐOẠN CODE XỬ LÝ XÓA SẢN PHẨM (Giữ nguyên logic của bạn)
// ==========================================================================
...
// ==========================================================================
// 3. XỬ LÝ LƯU ĐƠN HÀNG VÀO DATABASE KHI BẤM THANH TOÁN
// ==========================================================================
if (isset($_POST['checkout'])) {
    if (!isset($_POST['payment'])) {
        echo "<script>alert('Vui lòng chọn một phương thức thanh toán trước khi xác nhận!');</script>";
    } else {
        $payment_method = $_POST['payment'];
        $cart = isset($_SESSION['cart']) ? $_SESSION['cart'] : [];

        if (count($cart) > 0) {
            // Tính lại tổng tiền thực tế từ giỏ hàng để lưu vào DB
            $total_amount = 0;
            foreach ($cart as $item) {
                $total_amount += $item['price'];
            }

            // Giả định user_id = 1 (Nếu bạn đã làm chức năng đăng nhập, hãy đổi thành $_SESSION['user_id'])
            $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 1;

            try {
                // Bắt đầu một Transaction để đảm bảo an toàn dữ liệu (nếu lỗi sẽ hủy toàn bộ)
                $conn->beginTransaction();

                // Bước A: Lưu vào bảng `orders`
                $sql_order = "INSERT INTO orders (user_id, total_amount, payment_method, status) VALUES (?, ?, ?, 'Chờ xử lý')";
                $stmt = $conn->prepare($sql_order);
                $stmt->execute([$user_id, $total_amount, $payment_method]);
                
                // Lấy ID tự động tăng của đơn hàng vừa tạo xong
                $order_id = $conn->lastInsertId();

                // Bước B: Chạy vòng lặp lưu chi tiết từng món vào bảng `order_details`
                $sql_detail = "INSERT INTO order_details (order_id, product_name, price, image) VALUES (?, ?, ?, ?)";
                $stmt_detail = $conn->prepare($sql_detail);

                foreach ($cart as $item) {
                    $stmt_detail->execute([$order_id, $item['name'], $item['price'], $item['image']]);
                }

                // Hoàn tất Transaction lưu dữ liệu vào ổ đĩa
                $conn->commit();

                // Bước C: Xóa giỏ hàng hiện tại trong session và thông báo thành công
                unset($_SESSION['cart']);
                echo "<script>
                ...
                // Nếu xảy ra lỗi hệ thống, hoàn tác lại để không bị rác Database
                $conn->rollBack();
                echo "<script>alert('Có lỗi xảy ra trong quá trình xử lý đơn hàng: " . $e->getMessage() . "');</script>";
            }
        }
    }
}
...
<section class="cart">
    ...
    <div class="total">
        <h2>Tổng tiền: <?php echo number_format($total, 0, ',', '.'); ?>đ</h2>
    </div>
    ...
    <div class="payment">
        ...
        <form method="POST" action="cart.php">
            ...
            <label>
                <input type="radio" name="payment" value="Ví điện tử Momo">
                Ví điện tử Momo
            </label>
